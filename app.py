"""
Streamlit Web App for Heart Disease Prediction System (v2).
...
# Make `src/` importable so pickle can find the preprocessor / ensemble classes.
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
_SRC = os.path.join(BASE_DIR, "src")
if _SRC not in sys.path:
    sys.path.insert(0, _SRC)
# joblib unpickling needs the module already importable under its original name
import data_preprocessing  # noqa: F401  (registers HeartDiseasePreprocessor for pickle)
...
def page_predict():
    ...
    if preprocessor is None:
        st.error(
            "Preprocessor not found. Run `python main.py` first to generate "
    ...
    if submitted:
        with st.spinner("Running prediction..."):
            raw = {
                ...
                "cp": cp, "trestbps": float(trestbps),
                "chol": float(chol), "fbs": fbs,
                "restecg": restecg, "thalach": float(thalach),
                "exang": exang, "oldpeak": float(oldpeak),
                "slope": slope, "ca": ca, "thal": thal,
            }
            scaled = preprocessor.transform_patient(raw)
        ...
        raw_numeric = {
            "age": float(age), "trestbps": float(trestbps),
            "chol": float(chol), "thalach": float(thalach),
...
def page_compare():
    ...
    st.write("Side-by-side performance of all models on the UCI Heart Disease test set.")
...
Missing values imputed via KNN, duplicates removed, nominal features one-hot
encoded, continuous features Z-score normalised, data split 80/20 stratified.
...
hidden-layer sizes, learning rate, L2 regularisation and activation.
Fitness is **3-fold cross-validation accuracy**, tournament selection,
two-point crossover, Gaussian mutation, with elitism.

**4. PSO Optimization (PySwarms)**

Particle Swarm Optimization positions particles in a 3-D space
(learning rate, L2 regularisation, momentum). Particles converge via
social and cognitive acceleration. Fitness is CV accuracy.

**5. Hybrid GA \u2192 PSO \u2192 ANN**

GA returns the top-K architectures (not just one). PSO fine-tunes the
training parameters for each; the best CV score wins and is retrained
as the final model.

**6. XGBoost ceiling reference**

Gradient-boosted decision trees — a strong non-ANN baseline so we know
what accuracy is realistically achievable on this dataset.

**7. Seed-Averaging Ensemble**

The best Hybrid configuration is retrained with 5 different random
seeds; predictions are averaged to reduce seed-lottery variance.
""")
    ...
    with c1:
        st.write(
            "**Single-split evaluation** \u2014 every model evaluated on the same "
    ...
    with tc1:
        st.markdown(
            "**Machine Learning**\n"
            "- scikit-learn (MLPClassifier, KNNImputer)\n"
